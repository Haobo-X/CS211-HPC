...
void optimal_check(const double* A, const double* B, double* C, const int n, const int b)
{
    int i = 0;
    for (i = 0; i < n; i += b)
    {
...
inline void mydgemm_sub_best(const double* A, const double* B, double* C, int m, int rowsize, int b)
{
    ...
    for (i = 0; i < m; i += b)
    {
        ...
        for (j = 0; j < m; j += b)
        {
          ...
          for (k = 0; k < m; k += b)
          {
            register int k1bound = k + b > m? m : (k + b);
